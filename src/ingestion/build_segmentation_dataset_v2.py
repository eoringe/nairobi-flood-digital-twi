...
OUT_FILE = PROCESSED_DIR / "segmentation_dataset_v2.npz"
...
def main() -> None:
    print("=" * 72)
    print("REBUILDING SEGMENTATION DATASET (v2)")
    print("=" * 72)
    ...
    for ev in events:
        ...
        while day <= ev["end"]:
            ...
            # need SEQ_LEN days before t, and FORECAST_DAYS from t onward
            if i is None or i - SEQ_LEN < 0 or i + FORECAST_DAYS > len(rain):
                skipped += 1
                day += timedelta(days=STRIDE_DAYS)
                continue

            antecedent = rain[i - SEQ_LEN:i]              # [t-7 .. t-1]  (input)
            forecast = rain[i:i + FORECAST_DAYS]          # [t .. t+2]    (label only)

            y = flood_extent(score, float(forecast.sum()))

            rain_seqs.append(antecedent)
    ...
    print(f"        Antecedent rain (mm/day): mean {rain_arr.mean():.2f}, max {rain_arr.max():.1f}")
    ...
    np.savez_compressed(
        ...
        val_idx=idx["val"],
        test_idx=idx["test"],
        channel_names=np.asarray(meta["channel_names"] + [f"rain_t-{k}" for k in range(SEQ_LEN, 0, -1)]),
    ...
    print(f"       vs 6,020 MB for the old dataset — {6020/max(size_mb,0.1):.0f}x smaller")
